...
export default function DiscoverMoviesPage() {
  ...
  useEffect(() => {
    ...
    async function searchForMovie() {
      ...
      try {
        ...
        const url = `https://omdbapi.com/?apikey=c9cf54c2&s=${params.searchtext}`;
        ...
        setSearchState("Something went wrong. Try again or contact the host.");
      }
    }
  ...
  const clearResults = () => {
    history.push(`/discover/`); // to push the search term to params which is assigned useHistory and shows up after the / in url
    setSearchText("");
  };
